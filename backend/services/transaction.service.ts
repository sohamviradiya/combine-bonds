...
const TransactionService = (() => {
	const buyStock = async (portfolio: PortfolioInterface, transaction: Transaction): Promise<PortfolioInterface> => {
		...
		const stock_price = await StockModel.findById(transaction.stock, { price: 1 }).exec();
		await StockService.changeVolume(transaction.stock, transaction.amount);
		if (stockIndex === -1) {
			...
			portfolio.investments[stockIndex].quantity += Math.round(transaction.amount / stock_price.price);
		}
