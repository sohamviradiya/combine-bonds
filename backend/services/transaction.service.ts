...
const TransactionService = (() => {
	...
	const dumpStock = async (portfolio: PortfolioInterfaceWithID, stock_id: string) => {
		const stockIndex = portfolio.investments.findIndex((investment) => investment.stock == stock_id);

		if (stockIndex === -1) throw new Error("Stock not found in portfolio");

		const stock_quantity = portfolio.investments[stockIndex].quantity;
		const data = await StockModel.findById(stock_id).exec();
		const stock = {
			...data._doc,
			price: data.price,
		} as StockInterfaceWithID;
		stock.timeline[stock.timeline.length - 1].volume_in_market -= stock_quantity;
		const amount = stock_quantity * stock.price;

		portfolio.currentBalance += amount;
		portfolio.investments.splice(stockIndex, 1);
		stock.traders = stock.traders.filter((trader) => trader != portfolio._id);

		await StockModel.findByIdAndUpdate(stock_id, stock);
		return portfolio;
	};
	...
	return { buyStock, sellStock, deposit, withdraw, dumpStock };
})();
